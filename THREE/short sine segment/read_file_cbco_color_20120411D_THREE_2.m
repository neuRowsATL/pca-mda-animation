...
inc_sine_line_one = textscan(fid_inc_sine, '%s %s %s \n');
  ...
  %  ca = textscan(fid, '%f %f', 'Delimiter', '\n', 'CollectOutput', true);
    ...
    top_sine = inc_sine_data{1};
    
    
fid_no_sim = fopen('nosim.txt');
...
for ii = 1:size(projected_data, 2)
    ...
    % if ((row 1 column 1 of data file < time_data) & (time_data < r1c1 + r1c2)) |
     ...
     a = find( diff(sign(inf_sine_dec(:,1)-time_data)) == 2);
     b = find( diff(sign(low_sine(:,1)-time_data)) == 2);
     c = find( diff(sign(top_sine(:,1)-time_data)) == 2);
     d = find( diff(sign(no_sim(:,1)-time_data)) == 2);
     e = find( diff(sign(tugs_ol(:,1)-time_data)) == 2);
     f = find( diff(sign(inf_sine(:,1)-time_data)) == 2);

      if (time_data > inf_sine_dec(a,1)) & (time_data < inf_sine_dec(a,1) + inf_sine_dec(a,2))
         color1 = 'r' %red
     elseif (time_data > low_sine(b,1)) & (time_data < low_sine(b,1) + low_sine(b,2))
         color1 = 'm' %magenta
     elseif (time_data > top_sine(c,1)) & (time_data < top_sine(c,1) + top_sine(c,2))
         color1 = 'b' %black
...
%     symbol_1 = '*';
%          plot3(projected_data(1, ii), projected_data(2, ii), projected_data(3, ii), symbol_1, 'Color', color1, ...
%          'MarkerSize', 6);
     if ii > 1
